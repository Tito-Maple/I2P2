 ...
 * @return Move
 */
int AlphaBeta::AlphaBet(State* state, int depth, int alpha, int beta, bool maxPlayer) {
	...
	if (maxPlayer) {
		...
		for (auto& cur : state->legal_actions) {
			...
			maximum = std::max(maximum, AlphaBet(st, depth - 1, alpha, beta, false));
		...
		for (auto& cur : state->legal_actions) {
			State* st = state->next_state(cur);
			minimum = std::min(minimum, AlphaBet(st, depth - 1, alpha, beta, true));
